...
class TTTBot(private val level: Int, private val player: FieldsTwoPlayer) {
    ...
    private fun getDangerPosition(board: Array<Array<FieldsTwoPlayer>>): Pair<Int, Int>? {
        // ROWS
        repeat(3) {
            if ((countEntries(board[it], player) == 2) && (countEntries(board[it], player.getOpponent()) == 0))
        ...
        repeat(3) {
            if ((countEntries(turned[it], player) == 2) && (countEntries(turned[it], player.getOpponent()) == 0))
                return turned[it].lastIndexOf(FieldsTwoPlayer.EMPTY) to it
        }

        // DIAGONALS
        val d1 = arrayOf(board[0][0], board[1][1], board[2][2])
        if ((countEntries(d1, player) == 2) && (countEntries(d1, player.getOpponent()) == 0))
